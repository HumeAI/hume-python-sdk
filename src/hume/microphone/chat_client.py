...
from .audio_utilities import play_audio
from .microphone_sender import Sender
...
class ChatClient:
    ...
    async def _recv(self, *, socket: AsyncChatWSSConnection) -> None:
        async for socket_message in socket:
            ...
            if message["type"] in ["user_message", "assistant_message"]:
                ...
                raise ApiError(body=f"Error ({error_code}): {error_message}")
            elif message["type"] == "tool_call":
                print(
                    "Warning: EVI is trying to make a tool call. "
                    "Either remove tool calling from your config or "
                    "use the AsyncChatWSSConnection directly without a MicrophoneInterface."
                )
    ...
    async def run(self, *, socket: AsyncChatWSSConnection) -> None:
        """Run the chat client.
