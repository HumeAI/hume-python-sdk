...
class TestBatchJobDetails:
    ...
    def test_queued_status(self, queued_details: BatchJobDetails):
        assert queued_details.state.status == BatchJobStatus.QUEUED

    def test_completed(self, completed_details: BatchJobDetails):
        assert completed_details.state.status == BatchJobStatus.COMPLETED
    ...
    def test_job_time_completed(self, completed_details: BatchJobDetails):
        assert completed_details.state.get_created_time().strftime('%Y-%m-%d %H:%M:%S') == '2022-08-15 20:20:10'
        assert completed_details.state.get_started_time().strftime('%Y-%m-%d %H:%M:%S') == '2022-08-15 20:20:12'
        assert completed_details.state.get_ended_time().strftime('%Y-%m-%d %H:%M:%S') == '2022-08-15 20:20:15'
        assert completed_details.state.get_run_time_ms() == 3000

    def test_job_time_failed(self, failed_details: BatchJobDetails):
        assert failed_details.state.get_created_time().strftime('%Y-%m-%d %H:%M:%S') == '2022-08-15 20:20:11'
        assert failed_details.state.get_started_time().strftime('%Y-%m-%d %H:%M:%S') == '2022-08-15 20:20:14'
        assert failed_details.state.get_ended_time().strftime('%Y-%m-%d %H:%M:%S') == '2022-08-15 20:20:16'
        assert failed_details.state.get_run_time_ms() == 2000

    def test_job_time_queued(self, queued_details: BatchJobDetails):
        assert queued_details.state.get_created_time().strftime('%Y-%m-%d %H:%M:%S') == '2022-08-15 20:20:15'
        assert queued_details.state.get_started_time() is None
        assert queued_details.state.get_ended_time() is None
        assert queued_details.state.get_run_time_ms() is None
