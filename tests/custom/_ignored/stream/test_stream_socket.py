...
class TestStreamSocket:
    ...
    async def test_send_file(
        self, mock_face_protocol: Mock, tmp_path_factory: TempPathFactory
    ) -> None:
        configs = [FaceConfig(identify_faces=True)]
        socket = StreamSocket(mock_face_protocol, configs)
    ...
    async def test_send_facemesh_not_facemesh(
        self, mock_facemesh_protocol: Mock
    ) -> None:
        configs = [ProsodyConfig()]
        socket = StreamSocket(mock_facemesh_protocol, configs)
    ...
    async def test_send_facemesh_array_shapes(
        self, mock_facemesh_protocol: Mock
    ) -> None:
        configs = [FacemeshConfig()]
        socket = StreamSocket(mock_facemesh_protocol, configs)
        ...
        message = (
            "Number of MediaPipe landmarks per face must be exactly 478, found 474."
        )
        with pytest.raises(HumeClientException, match=re.escape(message)):
    ...
    async def test_invalid_payload_configs_send_text(
        self, mock_language_protocol: Mock
    ) -> None:
