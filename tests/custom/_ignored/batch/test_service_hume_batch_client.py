...
class TestServiceHumeBatchClient:
    ...
    def test_job_invalid_api_key(
        self, eval_data: EvalData, batch_client: HumeBatchClient
    ) -> None:
        data_url = eval_data["image-obama-face"]
    ...
    def test_local_file_upload_simple(
        ...
        self.check_job(
            job, config, FaceConfig, job_files_dirpath, complete_config=False
        )
    ...
    def test_data_as_raw_text(
        self, batch_client: HumeBatchClient, tmp_path_factory: TempPathFactory
    ) -> None:
        data_raw_text = "Test!"
        config = LanguageConfig()
        job_files_dirpath = tmp_path_factory.mktemp("job-files")
        ...
        self.check_job(
            job, config, LanguageConfig, job_files_dirpath, complete_config=False
        )

        predictions = job.get_predictions()
        language_predictions = predictions[0]["results"]["predictions"][0]["models"][
            "language"
        ]
    ...
    def test_local_file_upload_configure(
        ...
        language_results = predictions[0]["results"]["predictions"][0]["models"][
            "language"
        ]
    ...
    def check_details(
        self, job: BatchJob, details: BatchJobDetails, job_files_dirpath: Path
    ) -> None:
    ...
    def check_complete_config(
        ...
        class_fields = (
            set([field.name for field in fields(config_class)]) - exceptions_set
        )
        instance_fields = set(config.to_dict().keys()) - exceptions_set
        assert len(instance_fields) == len(
            class_fields
        ), "Model configuration must have all values set"
