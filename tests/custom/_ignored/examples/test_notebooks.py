...
class TestNotebooks:
    ...
    def test_example_notebook(
        ...
        utilities_import_code = (
            f'import sys; sys.path.append(\\"{example.dirpath}\\"); from utilities'
        )
        notebook_text = notebook_text.replace("from utilities", utilities_import_code)

        # Write temporary notebook file
        temp_notebook_filepath = (
            tmp_path_factory.mktemp("data-dir") / "temp-notebook.ipynb"
        )
    ...
    def test_notebook_empty_outputs(self, example: Example) -> None:
        with testbook(
            example.notebook_filepath, execute=False, kernel_name="hume"
        ) as notebook:
            for cell in notebook.cells:
                if "outputs" in cell:
                    assert (
                        len(cell["outputs"]) == 0
                    ), "All notebook cell outputs should be empty"
